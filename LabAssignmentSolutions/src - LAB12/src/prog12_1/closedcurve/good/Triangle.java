...
public class Triangle extends ClosedCurve {
		...
		public Triangle(double side1, double side2, double side3) throws IllegalClosedCurveException  {
			...
			double z = arr[2];
			if(x + y < z) {
				throw new IllegalTriangleException("IllegalTriangleException: Illegal sizes for a triangle: "+side1+", "+side2+", "+side3);
			}
			else {	
				setSides(x,y,z);
		...
		public void setSides(double x, double y, double z) throws IllegalClosedCurveException {
			if(x<0 || y<0 || z<0)
				throw new IllegalClosedCurveException("Sides of a triangle should be non-negative");
			if(x+y<z || y+z<x || z+x<y)
				throw new IllegalTriangleException("IllegalTriangleException: Illegal sides for a triangle");
			side1 = x;
			side2 = y;
			side3 = z;
		}

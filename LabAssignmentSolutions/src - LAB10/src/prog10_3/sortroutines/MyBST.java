...
public class MyBST {
	...
	public int[] readIntoArray(int size) {
		int[] array = new int[size];
		readIntoArrayHelper(root, array);
		return array;
	}
	
	private int i=0;
	public void readIntoArrayHelper(Node node, int[] arr) {
		if(node == null) return;
		readIntoArrayHelper(node.left, arr);
//		System.out.println(node.element+"\t"+ i);
		arr[i++] = node.element;
		readIntoArrayHelper(node.right, arr);
	}
